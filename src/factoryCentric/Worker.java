...
public class Worker 
{
	...
	private static PriorityQueue<Tile> karboniteQueue = new PriorityQueue<Tile>();
	
	public static void startTurn()
	{
		HashSet<Tile> depletedDeposits = new HashSet<Tile>();
		for (Tile deposit:GameInfoCache.allKarboniteDeposits)
		{
	...
	private static void buildOrder()
	{
		if (Game.round() == 1)
		{
			...
			if (buildDir != Direction.Center)
			{
				...
				karboniteQueue = new PriorityQueue<Tile>(Constants.HEIGHT * Constants.WIDTH / 4, new FactoryTileComparator());
				for (Tile deposit:GameInfoCache.allKarboniteDeposits)
				{
	...
	private static void giveWorkersOrders()
	{
		workerLabel: for (Robot worker:idleWorkers)
		{
			...
			{
				Tile[] deposits = new Tile[Constants.KARBONITEHARVESTCOUNT];
				for (int x = 0; x < Constants.KARBONITEHARVESTCOUNT; x++)
				{
					deposits[x] = karboniteQueue.poll();
				}
				Tile closest = null;
				int minDistance = Constants.INFINITY;
				if (GameInfoCache.allKarboniteDeposits.size() == -1)
				{
					continue;
				}
				for (Tile deposit:deposits)
				{
					if (deposit == null)
					{
						break;
